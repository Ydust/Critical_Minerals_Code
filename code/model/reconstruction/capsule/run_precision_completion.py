...
def main():
    ...
    manifest=json.loads((HERE/'PRECISION_COMPLETION_INPUT_MANIFEST.json').read_text())
    ...
    report=dict(accepted=bool(all(required)),branch=ARGS.branch,draw=ARGS.draw,rho=ARGS.rho,stopping_tolerance=1e-13,share_acceptance_unchanged=1e-6,weights_exact=weights_exact,scientific_categories_vs_original=category_mismatches,checks=checks,reporting_v2_equivalence_passed=bool(reporting),reporting_label_mismatches=mismatches,amount_difference_as_baseline_share=amount_error,independent_validation=physical,groups=count,unit_years=int(maincases.scenario.eq('baseline').sum()),elapsed_seconds=time.monotonic()-start,blocked_external_reads=blocked,input_manifest_sha256=hashlib.sha256((HERE/'PRECISION_COMPLETION_INPUT_MANIFEST.json').read_bytes()).hexdigest(),original_failures_preserved=True,reference_scope='Original-input high-precision model, not frozen low-precision solution' if ARGS.branch=='rebuilt' else 'Physical/unchanged-scientific-result validation; old local numerical differences remain diagnostic',full_pipeline_complete=False)
