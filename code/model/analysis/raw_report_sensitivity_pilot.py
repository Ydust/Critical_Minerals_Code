...
OUT=u.HERE/'results/raw_report_sensitivity_pilot'
...
def main():
    ...
    contract={str(p.relative_to(u.ROOT)):u.s.sha(p) for p in sources+[u.Path(__file__),u.Path(h.__file__),RAW/'SUMMARY.json',REF/'annual_indicators.csv.gz',REF/'all_windows.csv.gz']}
    ...
    protocol=dict(pilot_draws=2,temporal_rhos=[0.,.8],source_report_log_sd=.1,shared_reporter_variance_fraction=.5,
    ...
    all_rows=[]
    for rho in [0.,.8]:
        dest=OUT/f'rho_{rho:.1f}_draw_001'; dest.mkdir(exist_ok=True)
        marker=dest/'complete.json'
        if marker.exists():
            all_rows.extend(pd.read_csv(dest/'window_summaries.csv').to_dict('records')); continue
        start=time.monotonic()
        shared=h.ar_errors(np.random.default_rng(202609092001),len(reporter_codes),rho)
        panels=[]
        for source in sources:
            ...
            if not p.exists():
                ...
                rng=np.random.default_rng(202609092001+code)
            ...
            panels.append(panel)
            print(f'Raw report pilot rho={rho}: {name} reconstructed',flush=True)
        ...
        cats=h.classify(matrix,pair); rows=h.summarize(cats,list(h.selections(windows)),1,rho)
        pd.DataFrame(rows).to_csv(dest/'window_summaries.csv',index=False); all_rows.extend(rows)
        marker.write_text(json.dumps(dict(rho=rho,draw=1,unit_years=len(annual),product_rows=len(trade),
        ...
        print(f'Raw report pilot rho={rho}: classification complete',flush=True)
    pd.DataFrame(all_rows).to_csv(OUT/'all_pilot_window_summaries.csv',index=False)
    print('Two source-report-to-classification pilots complete, not full uncertainty inference.',flush=True)
