...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--draws-per-design',type=int,default=20)
    ...
    contract={str(p.relative_to(u.ROOT)):u.s.sha(p) for p in sources+[u.Path(__file__),u.Path(h.__file__),u.HERE/'BATCH_PROTOCOL_20260909.md',u.HERE/'frozen/build_model_inputs.py',u.HERE/'frozen/reconstruct_dynamic_panel.py',u.HERE/'results/routes.csv',u.s.ARCHIVE,RAW/'SUMMARY.json',REF/'annual_indicators.csv.gz',REF/'all_windows.csv.gz']}
    ...
    protocol=dict(requested_draws=2*args.draws_per_design,temporal_rhos=[0.,.8],source_report_log_sd=.1,shared_reporter_variance_fraction=.5,
    ...
    for draw,rho in [(draw,rho) for draw in range(1,args.draws_per_design+1) for rho in [0.,.8]]:
        ...
        marker.write_text(json.dumps(dict(rho=rho,draw=draw,unit_years=len(annual),product_rows=len(trade),
