...
import json
import raw_report_sensitivity_batch as p
...
def main():
    ...
    for marker in sorted(p.OUT.glob('*/complete.json')):
        ...
        rng=np.random.default_rng(202609093000+meta['draw'])
        ...
        for metal,year in [('Nickel',2018),('Copper',2024)]:
            ...
            reports.append(dict(draw=meta['draw'],rho=rho,metal=metal,year=year,unit_years=len(actual),errors=errors))
            print(f'Independent raw-draw origin/quality/route check: rho={rho}, {metal}/{year} passed',flush=True)
    assert len(reports)==12
    (p.OUT/'independent_flow_spot_checks.json').write_text(json.dumps(dict(accepted=True,scope='Two mineral-years at draws 2,10,20 in each temporal design; not exhaustive',checks=reports),indent=2),encoding='utf-8')
